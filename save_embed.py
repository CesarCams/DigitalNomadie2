...
batch_size = 64  # Choose an appropriate batch size based on your available memory
all_embeddings = []

for start_idx in range(0, len(df), batch_size):
    batch_tweets = list(df["tweet"].values[start_idx:start_idx + batch_size])
    inputs = tokenizer(batch_tweets, padding=True, truncation=True, max_length=64, return_tensors="pt")
    with torch.no_grad():
        outputs = model(**inputs)
    batch_embeddings = outputs.last_hidden_state[:, 0, :]  # Shape: (batch_size, 768)
...
features_matrix_train = torch.cat(all_embeddings, dim=0)
